...
class MultiKMeans:
    ...
    def fit(self, data):
        ...
        self.root = self._fit_recursive(data, data_in, self.n_clusters)

    def _fit_recursive(self, data, data_in, n_clusters):
        if len(n_clusters) == 0:
            return {"centroid": None, "data": data, "input": data_in}
        ...
        for i in range(n_clusters[0]):
            ...
            cluster_data_in = data_in[kmeans.labels_ == i]
            clusters.append(self._fit_recursive(cluster_data, cluster_data_in, n_clusters[1:]))
...
def print_tree(node, level=0):
    ...
    if "sub_clusters" in node:
        for sub_cluster in node["sub_clusters"]:
            print_tree(sub_cluster, level + 1)
...
def plot_tree_2d(node, level=0):
    ...
    if "centroid" in node and node["centroid"] is not None:
        ...
        print("  " * level + f"Level {level}: Leaf Node, Length {len(node['input'])}")
        plot_data = node["input"].cpu().numpy()
        plt.scatter(plot_data[:, 0], plot_data[:, 1], s=1)

    # If the current node has sub-clusters, recursively print each sub-cluster
    if "sub_clusters" in node:
        for sub_cluster in node["sub_clusters"]:
            plot_tree_2d(sub_cluster, level + 1)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        data = means + 0.1 * torch.randn(seq_len, n, device=device)  # Example data: 100 samples, 5 features each
