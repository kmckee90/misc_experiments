...
n_training_iters_autoencoder = 3000
...
class lateral_model(nn.Module):
    def __init__(self):
        super().__init__()
        self.v = mlp(mD_u + mD_v + mD_u * mD_v, mD_v, hidden_size=1024)
        self.dv = mlp(mD_u + mD_v + mD_u * mD_v, mD_v * mD_u, hidden_size=1024)
...
class preprocessor(nn.Module):
    def __init__(self):
        super().__init__()
        self.x_to_u = mlp(D_x, mD_u)
        self.u_to_v = mlp(mD_u, mD_v, hidden_size=1024)
        self.v_to_y = mlp(mD_v, D_y)
...
optim_uv = torch.optim.Adam(vnet.parameters(), lr=3e-4)
...
for i in range(n_training_iters):
    ...
    mse_uv = torch.nn.functional.mse_loss(Vv.squeeze(), Vu.squeeze().detach())
