...
#Forward
@jax.jit
def forward_r(key, X, Y, WSi, WSo, WBi, WS, WB):#, temp=1.0, lr=0.001, decay=0.0):
    w_decay = 1-decay
    Si = X.flatten() #Input
    Bi = Y.flatten() #Target
    S = jnp.zeros((L, d_h))
    ...
    key, subkey = random.split(key)
    h = jnp.float32(random.bernoulli(key, h))
    S = S.at[0].set(h)
    ...
    WSi = w_decay*WSi + lr*(jnp.outer(Si, B[-1]) - jnp.outer(Si, S[0])) #SB-SS
    WSo = w_decay*WSo + lr*(jnp.outer(S[-1], Bi) - jnp.outer(S[-1], So)) #SB-SS
    WSo = w_decay*WSo + lr*(jnp.outer(B[0], Bi) - jnp.outer(S[-1], So)) #BB-SS
    WBi = w_decay*WBi + lr*(jnp.outer(Bi, B[0]) - jnp.outer(So, S[-1])) #BB-SS
    WS = w_decay*WS + lr*(jax.vmap(jnp.outer)(jnp.flip(B,0)[:-1], jnp.flip(B, 0)[1:]) - jax.vmap(jnp.outer)(S[:-1], S[1:])) #SB-SS
    WS = w_decay*WS + lr*(jax.vmap(jnp.outer)(S[:-1], jnp.flip(B, 0)[1:]) - jax.vmap(jnp.outer)(S[:-1], S[1:])) #BB-SS
    WB = w_decay*WB + lr*(jax.vmap(jnp.outer)(B[:-1], B[1:]) - jax.vmap(jnp.outer)(jnp.flip(S, 0)[:-1], jnp.flip(S, 0)[1:]))#BB-SS
    return So, Bi, B, S, WSi, WSo, WBi, WS, WB


#Forward
@jax.jit
def forward_c(key, X, Y, WSi, WSo, WBi, WS, WB):#, temp=1.0, lr=0.001, decay=0.0):
    w_decay = 1-decay
    Si = X.flatten() #Input
    Bi = Y.flatten() #Target
    S = jnp.zeros((L, d_h))
...
# initial_sd = 0.5
# _WS = (random.uniform(key, (L-1, d_h, d_h))*2-1)*initial_sd
# key, subkey = random.split(key)
# _WB = (random.uniform(key, (L-1, d_h, d_h))*2-1)*initial_sd
# key, subkey = random.split(key)
# _WSi = (random.uniform(key, (d_i, d_h))*2-1)*initial_sd
# key, subkey = random.split(key)
# _WSo = (random.uniform(key, (d_h, d_o))*2-1)*initial_sd
# key, subkey = random.split(key)
# _WBi = (random.uniform(key, (d_i, d_h))*2-1)*initial_sd
...
for iter in range(500000):
    ...
    key, subkey = random.split(key)
    _So, _Bi, B, S, _WSi, _WSo, _WBi, _WS, _WB = forward_r(key, _X, _Y, _WSi, _WSo, _WBi, _WS, _WB)
