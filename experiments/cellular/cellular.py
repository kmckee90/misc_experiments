...
def gen_data(key):
    ...
    idx = random.randint(key, (2,), 0, dim)
    x = x.at[idx[0],:].set(1)
    x = x.at[:,idx[1]].set(1)
    return x
...
def forward_r(key, X, Y, WSi, WSo, WBi, WS, WB):#, temp=1.0, lr=0.001, decay=0.0):
    ...
    WSo = w_decay*WSo + lr*(jnp.outer(B[0], Bi) - jnp.outer(S[-1], So))
    WBi = w_decay*WBi + lr*(jnp.outer(Bi, B[0]) - jnp.outer(So, S[-1]))      
    WS = w_decay*WS + lr*(jax.vmap(jnp.outer)(jnp.flip(B, 0)[:-1], jnp.flip(B, 0)[1:]) - jax.vmap(jnp.outer)(S[:-1], S[1:]))
...
for iter in range(500000):
    ...
    key, subkey = random.split(key)
    _So, _Bi, B, S, _WSi, _WSo, _WBi, _WS, _WB = forward_c(key, _X, _Y, _WSi, _WSo, _WBi, _WS, _WB)
