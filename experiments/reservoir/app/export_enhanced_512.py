...
class EnhancedSpiking512(nn.Module):
    """
    Enhanced spiking neural network at 512x512 resolution.
    """
    def __init__(self):
        super().__init__()
    ...
    def forward(self, V0, S0, U, decay, thr, reset, input_split,
        ...
        # Global connections (simplified as average pooling)
        ...
        # Apply dropout to spikes (deterministic for ONNX compatibility)
        if drop_prob > 0:
...
def export_enhanced_512_onnx():
    """Export enhanced spiking neural network at 512x512 resolution."""

    model = EnhancedSpiking512().eval()
    ...
    torch.onnx.export(
        model,
        example_inputs,
        "spiking_step_enhanced_512.onnx",
    ...
    print("✓ Exported enhanced ONNX model to spiking_step_enhanced_512.onnx")
    print("✓ Resolution: 512x512")
    print("✓ Includes: dropout, local/global exc/inh connections")

if __name__ == "__main__":
    export_enhanced_512_onnx()