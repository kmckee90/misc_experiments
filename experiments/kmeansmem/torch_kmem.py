...
class TorchKMeans:
    ...
    def fit(self, data):
        ...
        self.centroids = data.mean() + torch.randn_like(sub) * 0.001
...
class MultiKMeans:
    def __init__(self, n_clusters, max_iter, device="cuda"):
        """
    ...
    def _fit_recursive(self, data, n_clusters):
        ...
        kmeans = TorchKMeans(n_clusters[0], max_iter=self.max_iter)
        kmeans.fit(data)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        x = data[torch.randint(0, int(1e6), (1,))].squeeze()
        x_hat = x.clone()
        x_hat += torch.randn_like(x_hat) * 0.1
        x_hat[:128] = 0
        m = mkmeans.recall(x_hat)
        torch.corrcoef(torch.stack((x, m)))

        # Sample around the memory if nothing familiar
        x_bar = x_hat * (1 + torch.randn_like(x_hat) * 0.3)
        m = mkmeans.recall(x_bar)
        torch.corrcoef(torch.stack((x, m)))
        torch.corrcoef(torch.stack((x_bar, x_hat)))

        # Compare with usual dot prod
        m_dp = data[torch.argmax(torch.cosine_similarity(data, x_hat.reshape(1, -1)))]
        torch.corrcoef(torch.stack((x, m_dp)))
