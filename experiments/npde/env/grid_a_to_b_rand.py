...
# Randomization incorporated into A2B:
# Random locations of the goal squares
...
class A_goal(Obj.Wall):
    def __init__(self):
        super().__init__(color="yellow")


class B_goal(Obj.Wall):
    def __init__(self):
        super().__init__(color="blue")


class C_goal(Obj.Wall):
    def __init__(self):
        super().__init__(color="red")


class D_goal(Obj.Wall):
    def __init__(self):
        super().__init__(color="green")
...
class AtoB(MiniGridEnv):
    def __init__(
        ...
        **kwargs,
        ...
        self.goal_tiles_walls = [A_goal, B_goal, C_goal, D_goal]
        ...
        self.current_goal_set = self.train_pairs[0]
        self.current_goal = self.current_goal_set[0]
        self.met_goal = [False, False]
        self.all_coords = np.array([[a, b] for a in range(2, self.height - 2) for b in range(2, self.height - 2)])
    ...
    def _gen_grid(self, width, height):
        ...
        self.goal_locs = np.random.permutation(self.all_coords)[:4]
        self.goal_locs = [(a[0], a[1]) for a in self.goal_locs]
        for i in range(len(self.goal_tiles)):
            self.grid.set(self.goal_locs[i][0], self.goal_locs[i][1], self.goal_tiles[i]())

        self.grid.set(width - 3, 0, self.goal_tiles_walls[self.current_goal_set[0]]())
        self.grid.set(width - 2, 0, self.goal_tiles_walls[self.current_goal_set[1]]())
